...
- `GET /:league/top-performances` — NBA only in v1 (returns 400 for nfl/nhl). Query params `?days=7` (1-30, clamped), `?type=games|cumulative` (required), `?limit=5` (1-25, clamped). Final-only leaderboard. `type=games`: best single-game performances in window — each entry has `{ player, game, rating, ratingGrade, stats: { points, rebounds, assists } }`; click target is `/:league/games/:gameId`. `type=cumulative`: per-player sum over window — each entry has `{ player, totalRating, gamesPlayed, avgPerGame, bestGame }`; click target is `/:league/players/:slug`. Cached 60s (`top-performances:{league}:{type}:{days}:{limit}`). Filters preseason/exhibition; tiebreaker `s.playerid ASC`.
- `GET /search`
