...
Favorites are displayed in a slide-in overlay panel (`FavoritesPanelContext` + `FavoritesPanel`) instead of inline on the Homepage. The panel is triggered by a star icon in the Navbar (visible when logged in) and is accessible from any page. It follows the same context + `AnimatePresence` pattern as the settings drawer and chat panel. `FavoritePlayersSection` and `FavoriteTeamsSection` accept a `compact` prop for the narrow panel layout — compact mode shows inline stat/game rows (limited to 2 per item) instead of full `StatCard`/`GameCard` components. Compact player stat rows link to `?tab=analysis#playerSlug` to scroll to the player in the box score. Each favorite player renders a small `PlayerStatusBadge` (size `sm`) inline next to position/jersey — source columns `players.status` + `players.status_description` are pulled by `favoritesService.getFavorites` and passed through to the UI. The panel auto-closes on route change, and mutually excludes with the settings drawer and chat panel (opening one closes the others). Z-index: backdrop `z-[69]`, panel `z-[70]`.
...
- Live-game ratings update each `liveSync` cycle as plays come in. As of 2026-05-09 the leaderboard query in `topPerformancesService.js` matches both Final and live status (`%final%` OR `%In Progress%` OR `%End of Period%` OR `%Halftime%`) so live games surface in `/:league/top-performances` for `performances`, `rankings`, and `plays`. Live rows expose `game.isLive: true` plus current `homeScore`/`awayScore`, and force `result: null` so a provisional W/L isn't shown — frontend rows render a `LIVE` pill in place of the W/L badge. Players who haven't touched the ball yet have `stats.rating IS NULL` and are filtered out, so no minimum-period floor is needed.
