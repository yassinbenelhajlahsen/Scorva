...
export default function PlayerCard({ player, players, league}) {
  const getPlayerImage = (playerName) => {
    const playerData = players.find(p => p.name.includes(playerName));
    return playerData && playerData.image ? playerData.image : null;
  };
  ...
  <div className="relative border border-zinc-700 bg-zinc-800 p-6 text-center rounded-xl shadow-lg transition-transform duration-200 hover:scale-105 cursor-pointer max-w-md min-w-[200px] h-48 flex flex-col items-center justify-center">
    <img
      src={getPlayerImage(player.name) || "https://cdn.nba.com/headshots/nba/latest/1040x760/2374.png"}
      alt={player.name}
