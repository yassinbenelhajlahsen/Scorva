...
export default function PlayerHero({
  ...
  // When rankings are present we left-align everything so the rankings column
  // can sit cleanly beside the info column (on mobile and desktop alike).
  const infoAlign = hasRankings
    ? "items-start"
    : "items-center md:items-start";
  const textAlign = hasRankings ? "text-left" : "text-center md:text-left";
  const rowJustify = hasRankings
    ? "justify-start"
    : "justify-center md:justify-start";

  const infoBlock = (
    <div className={`flex flex-col gap-3 ${infoAlign} min-w-0 flex-1`}>
      <div className="flex items-center gap-3">
        <h1
          className={`text-3xl sm:text-4xl font-bold tracking-tight text-text-primary ${textAlign}`}
      ...
      {showStatus && (status || streak) && (
        <div className={`flex flex-wrap items-center ${rowJustify} gap-2`}>
          <PlayerStatusBadge status={status} title={statusDescription || undefined} />
  ...
  return (
    <div className="flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8 mb-8">
      ...
      {hasRankings ? (
        <div className="flex flex-row items-start gap-4 md:gap-6 w-full md:flex-1 min-w-0">
          {infoBlock}
